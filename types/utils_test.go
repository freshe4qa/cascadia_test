...
func init() {
	cfg := sdk.GetConfig()
	cfg.SetBech32PrefixForAccount("cascadia", "cascadiapub")
}
...
func TestGetCascadiaAddressFromBech32(t *testing.T) {
	testCases := []struct {
		name       string
		address    string
		expAddress string
		...
		{
			"cascadia address",
			"cascadia1qql8ag4cluz6r4dz28p3w00dnc9w8ueuafmxps",
			"cascadia1qql8ag4cluz6r4dz28p3w00dnc9w8ueuafmxps",
			false,
		},
		{
	...
	for _, tc := range testCases {
		addr, err := GetCascadiaAddressFromBech32(tc.address)
		if tc.expError {
