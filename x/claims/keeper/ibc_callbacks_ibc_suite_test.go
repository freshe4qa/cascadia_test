...
func (suite *IBCTestingSuite) SetupTest() {
	...
	coins := sdk.NewCoins(sdk.NewCoin("uCC", sdk.NewInt(10000)))
...
func (suite *IBCTestingSuite) TestOnAcknowledgementPacketIBC() {
	sender := "cascadia1sv9m0g7ycejwr3s369km58h5qe7xj77hvcxrms"
	receiver := "cascadia1hf0468jjpe6m6vx38s97z2qqe8ldu0njdyf625"
	...
	testCases := []struct {
		...
		},
		{
			...
			func(claimableAmount int64) {
				...
				coins := sdk.NewCoins(sdk.NewCoin("uCC", amt))
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			transfer := transfertypes.NewFungibleTokenPacketData("uCC", "100", sender, receiver)
			...
			coin := suite.chainA.App.(*app.Evmos).BankKeeper.GetBalance(suite.chainA.GetContext(), senderAddr, "uCC")
			suite.Require().Equal(sdk.NewCoin("uCC", sdk.NewInt(tc.expectedBalance)).String(), coin.String())
...
func (suite *IBCTestingSuite) TestOnRecvPacketIBC() {
	sender := "cascadia1hf0468jjpe6m6vx38s97z2qqe8ldu0njdyf625"
	receiver := "cascadia1sv9m0g7ycejwr3s369km58h5qe7xj77hvcxrms"
	...
	testCases := []struct {
		...
		},
		{
			...
			func(claimableAmount int64) {
				amt := sdk.NewInt(claimableAmount)
				coins := sdk.NewCoins(sdk.NewCoin("uCC", sdk.NewInt(claimableAmount/4)))
		...
		},
		{
			...
			func(claimableAmount int64) {
				amt := sdk.NewInt(claimableAmount)
				coins := sdk.NewCoins(sdk.NewCoin("uCC", amt.Add(amt.QuoRaw(2))))
		...
		},
		{
			...
			func(claimableAmount int64) {
				amt := sdk.NewInt(claimableAmount)
				coins := sdk.NewCoins(sdk.NewCoin("uCC", amt.QuoRaw(2)))
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			transfer := transfertypes.NewFungibleTokenPacketData("uCC", triggerAmt, sender, receiver)
			bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
			packet := channeltypes.NewPacket(bz, 1, path.EndpointA.ChannelConfig.PortID, path.EndpointA.ChannelID, path.EndpointB.ChannelConfig.PortID, path.EndpointB.ChannelID, timeoutHeight, 0)
			...
			coin := suite.chainB.App.(*app.Evmos).BankKeeper.GetBalance(suite.chainB.GetContext(), receiverAddr, "uCC")
			suite.Require().Equal(coin.String(), sdk.NewCoin("uCC", sdk.NewInt(tc.expectedBalance)).String())
