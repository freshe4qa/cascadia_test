...
var (
	now  = time.Now().UTC()
	acc1 = sdk.MustAccAddressFromBech32("cascadia1qxx0fdsmruzuar2fay88lfw6sce6emamyu2s8h4d")
	acc2 = sdk.MustAccAddressFromBech32("cascadia1nsrs4t7dngkdltehkm3p6n8dp22sz3mct9uhc8")
)
...
func (suite *GenesisTestSuite) TestClaimInitGenesis() {
	testCases := []struct {
		...
		},
		{
			...
			func() {
				coins := sdk.NewCoins(sdk.NewCoin("uCC", sdk.NewInt(2_800)))
		...
		},
		{
			...
			func() {
				coins := sdk.NewCoins(sdk.NewCoin("uCC", sdk.NewInt(400)))
				err := testutil.FundModuleAccount(suite.app.BankKeeper, suite.ctx, types.ModuleName, coins)
				suite.Require().NoError(err)
			},
			false,
